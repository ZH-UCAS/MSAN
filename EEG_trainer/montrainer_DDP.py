...
class Trainer:
    def __init__(self, model, loss1, loss2, source_train_loader, target_test_loader, args,train_sampler):
        self.model = model
        self.args = args
        ...
        self.schedular = torch.optim.lr_scheduler.StepLR(self.optimizer, 50, gamma=0.5, last_epoch=-1)
    ...
    def train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.start_epoch + self.args.num_epochs):
            ...
            if train_acc > self.max_train_acc:
                ...
                self.early_stop_timer = 0
            elif (train_acc > 0.98) & (train_loss < 0.1) & ((train_loss - self.best_loss) < 0.01):
            ...
            if self.early_stop_timer >= self.args.early_stop_patience:
                ...
                break
            if (train_acc > 0.999) & (train_loss < 0.001) & (self.early_stop_timer > 5):
                print("\nearly stop 2\n")
                print("epoch : {}\ntrain : acc {:.4} | loss {:.4} | early-stop count {}".format(epoch, train_acc, train_loss, self.early_stop_timer))
                break
            self.schedular.step()
